    ...
    <div class="min-h-screen h-full">
        ...
        <div class="flex">
            ...
            <div class="flex-grow h-full pl-[34px] pr-[88px] py-7 box-border mb-10 w-full" id="content">
                <div class="mb-9">
                    {!! Breadcrumbs::render('operator-izin-pendirian-berkas', $peruntukan = request('namasekolah')) !!}
                </div>
                {{-- CONTENT --}}
                ...
                <form action="{{ url('/dashboard-operator/izin-pendirian/upload-berkas') }}" method="GET"
                    ...
                    @if (request('peruntukan') === 'tk')
                        @include('operator.izin-pendirian.berkas-izin-pendirian.table-tk')
                    @elseif (request('peruntukan') === 'sd')
                        @include('operator.izin-pendirian.berkas-izin-pendirian.table-sd')
                    @elseif (request('peruntukan') === 'smp')
                        @include('operator.izin-pendirian.berkas-izin-pendirian.table-smp')
                    @else
                        @include('operator.izin-pendirian.berkas-izin-pendirian.table-sd')
                    @endif
