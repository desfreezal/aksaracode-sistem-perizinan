<div x-data="{ isOpen: false }" :class="isOpen ? 'w-80' : 'w-20'" class=" min-h-screen relative space-y-3 py-2 duration-500"
    ...
    <div x-show.transition.duration.500ms="isOpen" class="sidebar-content w-80 transition-all">
        <a href="/dashboard/daftar-ulang/data"
            class="cursor-pointer flex items-center pl-11 w-full  {{ Request::is('dashboard/daftar-ulang/data') || Route::currentRouteName() === 'pemohon-daftar-ulang' ? 'bg-white shadow' : '' }} hover:bg-white gap-x-3 py-2">
        ...
        <a href="#"
            class="cursor-pointer flex items-center pl-11 w-full {{ Route::currentRouteName() === 'pemohon-detail' ? 'bg-white shadow' : '' }} hover:bg-white hover:shadow gap-x-3 py-2">
